...
class DigitalTwin:
    ...
    Args:
        ...
        ensemble: Optional :class:`otwin.uq.Ensemble` instance. Required when
            ``uq='ensemble'``.
    ...
    def __init__(
        ...
        if isinstance(model, PortHamiltonianSystem):
            ...
            if model == "phnn":
                from otwin.learn.phnn import PortHamiltonianNN  # noqa: F401

                raise NotImplementedError(
                    "Pass a fitted PortHamiltonianNN (or a PortHamiltonianSystem) "
                    "as `model`; constructing a twin from the string 'phnn' is not "
                    "supported. Install the learning extra with: pip install otwin[torch]"
                )
            elif model == "gp-phs":
                raise NotImplementedError(
                    "Pass a fitted GP-PHS model as `model`. "
                    "Install the GP extra with: pip install otwin[gp]"
                )
            else:
