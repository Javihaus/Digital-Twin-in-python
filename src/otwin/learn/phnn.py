...
Requires the ``[torch]`` extra: ``pip install otwin[torch]``.
...
def _require_torch() -> Any:
    try:
        ...
        raise ImportError(
            "PortHamiltonianNN requires PyTorch. Install with: pip install otwin[torch]"
        ) from exc
...
class PortHamiltonianNN:
    ...
    def fit(
        ...
        """
        torch = self._torch
        from otwin.learn.losses import derivative_loss, passivity_penalty
