...
### First-principles: water tank (point forecast)
...
| **WaterTankModel (port-Hamiltonian)** | **0.0031** | **0.13** | **0.054** |

- **Skill score:** 0.94 (94% better than persistence over the horizon).
- This is a **point** forecast (no uncertainty) → no CRPS / PICP here; calibrated intervals are shown in the battery example.

### Empirical-law: battery State-of-Health (NASA fleet)

*Empirical fade-law prior + estimated residual + conformal bands. Temporal split (estimate on early cycles, forecast the remainder). Source of truth: [`examples/battery_soh`](examples/battery_soh) (`results.csv`, `figure_data/`).*

| Cell (24 °C) | RMSE (SoH) | Theil's U | CRPS |
|---|---|---|---|
...
- **Theil's U < 1** on all four 24 °C cells → the grey-box model beats persistence over the horizon.
- **Known limitation (reported as-is):** on the colder 4 °C cells (B0045–B0048) it does **not** yet beat persistence (Theil's U ≈ 1.1–1.8).
...
White-box structure preservation (energy, dissipation) with a leakage-free benchmark. See [`examples/water_tank_phs`](examples/water_tank_phs).
